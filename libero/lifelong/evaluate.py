...
def main():
    ...
    noise_tag = f"_noise{args.action_noise_std}" if args.action_noise_std > 0.0 else "_nonoise"
    video_folder = os.path.join(
        args.save_dir,
        f"{args.benchmark}_{args.algo}_{args.policy}_{args.seed}_load{args.load_task}_on{args.task_id}{noise_tag}_videos",
    )
    ...
    with Timer() as t, VideoWriter(video_folder, args.save_videos, single_video=False) as video_writer:
        ...
        env_num = args.num_procs
        env_fns = [lambda: OffScreenRenderEnv(**env_args) for _ in range(env_num)]
        if env_num == 1:
        ...
        for i in range(eval_loop_num):
            ...
            with torch.no_grad():
                while steps < cfg.eval.max_steps:
                    ...
                    if args.action_noise_std > 0.0:
                        actions += np.random.normal(0, args.action_noise_std, actions.shape)
                    ...
                    for k in range(env_num):
                        ...
                        if ep_idx < n_eval:
                            video_writer.append_obs(
                                obs[k], dones[k], idx=ep_idx, camera_name="agentview_image"
                            )
            ...
            for k in range(env_num):
                ...
                if ep_idx < n_eval:
                    ...
                    print(f"[info] episode {ep_idx:2d}: {'SUCCESS' if dones[k] else 'FAIL'}")
