...
def main(hydra_cfg):
    ...
    _max_tasks = cfg.get("max_tasks")
    _load_n = min(n_manip_tasks, _max_tasks) if _max_tasks is not None else n_manip_tasks
    for i in range(_load_n):
    ...
    if cfg.get("max_tasks") is not None:
