...
import numpy as np
import requests
# tensorflow removed — image ops replaced with PIL equivalents below
import torch
...
def resize_image_for_policy(img: np.ndarray, resize_size: Union[int, Tuple[int, int]]) -> np.ndarray:
    ...
    # Resize via PIL Lanczos to match RLDS training pipeline closely
...
def center_crop_image(image: Union[np.ndarray, Image.Image]) -> Image.Image:
    """Center crop (90% of area) and resize to OPENVLA_IMAGE_SIZE using PIL."""
    if isinstance(image, np.ndarray):
        image = Image.fromarray(image).convert("RGB")
    w, h = image.size
    crop_frac = 0.9 ** 0.5  # sqrt(0.9) for linear dimensions
    crop_w = int(w * crop_frac)
    crop_h = int(h * crop_frac)
    left = (w - crop_w) // 2
    top = (h - crop_h) // 2
    image = image.crop((left, top, left + crop_w, top + crop_h))
    image = image.resize((OPENVLA_IMAGE_SIZE, OPENVLA_IMAGE_SIZE), Image.LANCZOS)
